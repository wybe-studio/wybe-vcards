...
export const ColorPickerHue = ({
	...
	return (
		<Slider.Root
			...
			max={360}
			onValueChange={(values: number[]) => setHue(values[0] ?? 0)}
			step={1}
			value={[hue]}
			{...(props as any)}
		>
...
export const ColorPickerAlpha = ({
	...
	return (
		<Slider.Root
			className={cn("relative flex h-4 w-full touch-none", className)}
			max={100}
			onValueChange={(values: number[]) => setAlpha(values[0] ?? 0)}
			step={1}
			value={[alpha]}
			{...(props as any)}
		>
...
export const ColorPickerEyeDropper = ({
	...
	const handleEyeDropper = async () => {
		try {
			...
			const hslArr = color.hsl().array();

			setHue(hslArr[0] ?? 0);
			setSaturation(hslArr[1] ?? 0);
			setLightness(hslArr[2] ?? 0);
			setAlpha(100);
		} catch (error) {
